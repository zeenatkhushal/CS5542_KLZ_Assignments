...
object IPApp {
  ...
  def naivebayesModel(sc: SparkContext): Unit = {

    if (Files.exists(Paths.get(IPSettings.NAIVE_BAYES_PATH))) {
      println(s"${IPSettings.NAIVE_BAYES_PATH} exists, skipping Random Forest model formation..")
      return
    }
    ...
    val predictionAndLabel = test.map { point =>
      val prediction = naiveBayesModel.predict(point.features)
  ...
  def generateDecisionTree(sc: SparkContext): Unit = {
  ...
  def main(args: Array[String]) {
    ...
    /**
      ...
      *
      * This shall be used as a input to Decision Tree Classifier to create a model
      */
    createHistogram(sc, images)

    /**
      * From the labeled Histograms a Decision Tree Model is created
      */
    generateDecisionTree(sc)
